...
app.get('/getallagents', (req, res) => {
    ...
    connection.query(`SELECT * FROM agents ORDER BY agents.token ASC, agents.rank_order ASC`, (err, data) => {
        if(err) {
            console.log(err)
            res.json({
                code: '00401',
...
app.get('/getdata/:username', (req, res) => {
    ...
    connection.query(`SELECT * FROM agents WHERE username = '${username}' AND token = '${token}'`, (err, data) => {
        if(err) {
            // res.send(err)
            res.json({
